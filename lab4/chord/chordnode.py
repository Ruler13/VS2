    ...
    def run(self):
        while True:  # Start node operation loop
            ...
            if request[0] == constChord.LOOKUP_REQ:  # A lookup request
                ...
                if int(sender) in self.node_list: # Sender is a node in chord ring
                    client_id = request[2]
                    lookup = request[1]
                    next_id: int = self.local_successor_node(int(lookup))
                    # print(" LOOKUP:" + str(next_id))
                    if next_id == self.node_id:
                        # print("Sender is not client and node knows lookup")
                        self.channel.send_to([client_id], (constChord.LOOKUP_REP, next_id))
                    else:
                        # print("Sender is not client and node does not know lookup")
                        destination = str(next_id)
                        self.channel.send_to([destination], (constChord.LOOKUP_REQ, lookup, client_id))
                else: # Sender is the client
                    client_id = sender
                    # print(request[2])
                    lookup = request[1]
                    next_id: int = self.local_successor_node(int(lookup))
                    ...
                    if next_id == self.node_id:
                        # print("Sender is client and node knows lookup")
                        self.channel.send_to([client_id], (constChord.LOOKUP_REP, next_id))
                    else:
                        # ("Sender is client and node does not know lookup")
                        destination = str(next_id)
                        self.channel.send_to([destination], (constChord.LOOKUP_REQ, lookup, client_id))
