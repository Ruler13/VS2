...
class DummyChordClient:
    ...
    def run(self):
        ...
        lookup_node = random.randint(0,self.channel.MAXPROC)
        while lookup_node in list(self.channel.channel.smembers('node')):
            lookup_node = random.randint(0, self.channel.MAXPROC)
        # lookup_node = bytes(str(lookup_node),"ascii")
        msg = (constChord.LOOKUP_REQ, lookup_node)

        # Send a lookup message to a random node.
        self.channel.send_to({random_node.decode()}, msg)
        # Wait for respond and print it.
        receive = self.channel.receive_from_any()
        print(receive)
